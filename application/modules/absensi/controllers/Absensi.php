...
class Absensi extends BackendController {
	...
	{
		$this->data['total'] = $this->ion_auth->like('username', input_get('q'))->like('fullname', input_get('q'))->like('email', input_get('q'))->users('mahasiswa')->num_rows();
		...
		$this->data['end'] = ($this->data['total'] > 0 ? $this->data['pagination']->offset+$this->ion_auth->limit($this->data['pagination']->limit)->offset($this->data['pagination']->offset)->users('mahasiswa')->num_rows() : 0);
		$this->data['users'] = $this->ion_auth->like('username', input_get('q'))->like('fullname', input_get('q'))->like('email', input_get('q'))->limit($this->data['pagination']->limit)->offset($this->data['pagination']->offset)->order_by('id', 'DESC')->users('mahasiswa')->result();
		$this->data['message'] = $this->_show_message();

		$this->_render_page('mahasiswa/list', $this->data);
	}
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('username', 'nim', 'trim|required|is_unique[' . $tables['users'] . '.username]');
		...
		if ($this->form_validation->run() === TRUE && $id = $this->ion_auth->register($email, $password, $email, $additional_data)) {
			...
			$this->_render_page('mahasiswa/add', $this->data);
		}
	}
	...
	public function edit($id)
	{
		...
		if (!$user->num_rows()) redirect(base_url('mahasiswa'), 'refresh');
		...
		if (isset($_POST) && !empty($_POST)) {
			if ($user->id != wah_decode(input_post('id'))) show_error($this->lang->line('error_csrf'));
			...
			if ($this->form_validation->run() === TRUE) {
				...
				if ($this->ion_auth->update($user->id, $data)) {
					$this->_set_message('success', $this->ion_auth->messages());
					redirect(base_url('mahasiswa/edit/' . wah_encode($user->id)), 'refresh');
				} else {
		...
		$this->_render_page('mahasiswa/edit', $this->data);
	}
	...
	public function delete($id)
	{
		$user = $this->ion_auth->user(wah_decode($id));
		if (!$user->num_rows()) redirect(base_url('mahasiswa'), 'refresh');
		if ($user->row()->id === '1') redirect(base_url('mahasiswa'), 'refresh');
	...
	public function export_excel()
	{
		...
		$users = $this->ion_auth->order_by('id', 'DESC')->users('mahasiswa')->result();
	...
	public function export_pdf()
	{
		...
		$pdf = new Fpdf();
		$headers = array('NIM', 'Full Name', 'Email', 'Groups', 'Status');
